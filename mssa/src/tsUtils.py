...
def index_ts_mapper(start, interval, timestamp):
    ...
    if isinstance(start, (int, np.integer)):
        ...
        return ceil((timestamp.value-start.value)/(interval*10**9))
...
def index_ts_inv_mapper(start, interval, index):
    ...
    """
    if isinstance(start, (int, np.integer)):
        return ((index *interval) + start).astype(int)

    elif  isinstance(start, (pd.Timestamp)):
        return  pd.to_datetime(index*(interval*10**9)+start.tz_localize(None).value)
